...
# less_equal.py note). The scalar gate in ge_scalar already restricts the
# fast path to finite scalars exactly representable in A.dtype, so the
# in-dtype compare is bit-identical to torch's wrapped-scalar semantics for
# every non-NaN value (incl. the ge-vs-gt equality boundary x == s -> True).
# NaN boundary: the native unmasked compare gives NaN >= s == False (matches
# torch); the masked variant at production TILE gives NaN -> True, which
# differs from the saturating path's NaN -> False on masked shapes -- same
# documented tile artifact as le_scalar (NaN is outside the randn
# test/benchmark matrix; no regression in the covered matrix).
